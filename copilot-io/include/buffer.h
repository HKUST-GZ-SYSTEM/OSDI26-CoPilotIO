...
// Allocate hugepage-backed memory (2MB pages, physically contiguous).
// GPU can access via zero-copy (PCIe). Avoids CQR contiguity issues.
inline DmaPtr createDmaHugepage(const nvm_ctrl_t* ctrl, size_t size)
{
    nvm_dma_t* dma = nullptr;
    const size_t HUGE_PAGE_SIZE = 2UL * 1024 * 1024;
    size_t alloc_size = ((size + HUGE_PAGE_SIZE - 1) / HUGE_PAGE_SIZE) * HUGE_PAGE_SIZE;

    void* hostPtr = mmap(NULL, alloc_size, PROT_READ | PROT_WRITE,
                         MAP_PRIVATE | MAP_ANONYMOUS | MAP_HUGETLB, -1, 0);
    if (hostPtr == MAP_FAILED)
        throw error(string("Failed to mmap hugepage: ") + strerror(errno));

    memset(hostPtr, 0, size);

    cudaError_t cerr = cudaHostRegister(hostPtr, alloc_size, cudaHostRegisterMapped);
    if (cerr != cudaSuccess) {
        munmap(hostPtr, alloc_size);
        throw error(string("Failed to register hugepage with CUDA: ") + cudaGetErrorString(cerr));
    }

    int status = nvm_dma_map_host(&dma, ctrl, hostPtr, size);
    if (!nvm_ok(status)) {
        cudaHostUnregister(hostPtr);
        munmap(hostPtr, alloc_size);
        throw error(string("Failed to DMA-map hugepage: ") + nvm_strerror(status));
    }

    void* devPtr = nullptr;
    cerr = cudaHostGetDevicePointer(&devPtr, hostPtr, 0);
    if (cerr != cudaSuccess) {
        nvm_dma_unmap(dma);
        cudaHostUnregister(hostPtr);
        munmap(hostPtr, alloc_size);
        throw error(string("Failed to get device pointer for hugepage: ") + cudaGetErrorString(cerr));
    }

    dma->vaddr = devPtr;

    return DmaPtr(dma, [hostPtr, alloc_size](nvm_dma_t* dma) {
        nvm_dma_unmap(dma);
        cudaHostUnregister(hostPtr);
        munmap(hostPtr, alloc_size);
    });
...
// Allocate CPU pinned memory with DMA host mapping (4KB pages).
// GPU can access via zero-copy (PCIe). vaddr is set to GPU-visible pointer.
inline DmaPtr createDmaPinned(const nvm_ctrl_t* ctrl, size_t size)
{
    nvm_dma_t* dma = nullptr;
    void* hostPtr = nullptr;

    cudaError_t cerr = cudaHostAlloc(&hostPtr, size, cudaHostAllocMapped);
    if (cerr != cudaSuccess)
    {
        throw error(string("Failed to allocate pinned memory: ") + cudaGetErrorString(cerr));
    }
    memset(hostPtr, 0, size);
    ...
    if (!nvm_ok(status))
    {
        ...
        throw error(string("Failed to DMA-map pinned memory: ") + nvm_strerror(status));
    }
    ...
    if (cerr != cudaSuccess)
    {
        nvm_dma_unmap(dma);
        cudaFreeHost(hostPtr);
        throw error(string("Failed to get device pointer for pinned memory: ") + cudaGetErrorString(cerr));
    }

    dma->vaddr = devPtr;

    return DmaPtr(dma, [hostPtr](nvm_dma_t* dma) {
        nvm_dma_unmap(dma);
        cudaFreeHost(hostPtr);
    });
}
